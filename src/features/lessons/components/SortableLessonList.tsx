...
export function SortableLessonList({ sections, lessons }: {
    ...
    lessons: { id: string, name: string, status: LessonStatus, youtubeVideoId: string, description: string | null, sectionId: string }[]
}) {
    return (
        <SortableList items={lessons} onOrderChange={updateLessonOrders}>
            {
                items => items.map(lesson => (
                    <SortableItem key={lesson.id} id={lesson.id} className="flex items-center gap-1">
                        ...
                        <LessonsFormDialog lesson={lesson} sections={sections} >
