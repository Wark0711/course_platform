...
import { getPublicProductForPurchase, userOwnsProduct } from "@/features/products/db/products";
...
async function SuspendedComponent({ params, searchParams }: { params: Promise<{ productId: string }>, searchParams: Promise<{ authMode: string }> }) {
    const { productId } = await params
    const { user } = await getCurrentUser({ allData: true })
    const product = await getPublicProductForPurchase(productId)
    ...
    return (
        <div className="container my-6 flex flex-col items-center">
            ...
            {
                isSignUp
                    ? <SignUp routing="hash" signInUrl={`/products/${productId}/purchase?authMode=signIn`} forceRedirectUrl={`/products/${productId}/purchase`} />
                    : <SignIn routing="hash" signUpUrl={`/products/${productId}/purchase?authMode=signUp`} forceRedirectUrl={`/products/${productId}/purchase`} />
            }
        </div>
    )
}