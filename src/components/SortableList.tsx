...
export function SortableList<T extends { id: string }>({ items, onOrderChange, children }: {
    items: T[],
...
export function SortableItem({ id, children, className }: { id: string, children: ReactNode, className: string }) {
    ...
    return (
        <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn('flex gap-2 items-center bg-background rounded-lg p-2', isActive && 'z-10 border shadow-md')}>
